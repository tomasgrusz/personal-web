  ...
  return (
    <section className={styles.EndorsementSection}>
      ...
      <p className={styles.description}>
        Grateful to have worked with such talented people. Here&apos;s a glimpse of their thoughts
        on our time together and the experiences we&apos;ve shared.{' '}
        <Link
          href={'https://www.linkedin.com/in/tomasgrusz/details/recommendations/'}
          target="_blank"
          aria-label="LinkedIn endorsements"
        >
          Discover all endorsements
        </Link>
        .
      </p>
      <Carousel>
        {endorsements.slice(0, 5).map((endorsement, index) => {
          return (
            <EndorsementCard
              key={index}
              {...endorsement}
              companyIcon={PartnerImages[endorsement.company]}
            />
          );
        })}
      </Carousel>
