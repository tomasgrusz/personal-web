...
const Scene = ({ enabled }: { enabled: boolean }) => {
  ...
  let portalLightTarget = new Mesh(new CylinderGeometry(0, 0, 0, 0), new MeshBasicMaterial({}));
  ...
  return (
    <group position={[3, 1, 0]}>
      <Float speed={2} rotationIntensity={0.1} floatIntensity={1}>
        ...
        {/* Light */}
        <spotLight color={'#aaf'} intensity={0.5} angle={Math.PI / 2} position={[-15, 15, -10]} />
        <spotLight color={'#aaf'} intensity={0.75} angle={Math.PI / 4} position={[25, -2, 20]} />
      </Float>
      <ambientLight color={'#70f'} intensity={0.2} />
      <PerspectiveCamera makeDefault position={[25, 20, 35]} zoom={0.75} />
      <OrbitControls target={[4, 4, 0]} enablePan={false} enableZoom={false} enableRotate={false} />
