...
import { Logo as Three } from "./Logo";
...
const Model = () => {
  ...
  const [rotationVector, setRotationVector] = useState<Euler>(
    new Euler(0, 0, 0, "XYZ")
  );
  ...
  useFrame(() => {
    setRotationVector(
      new Euler(
        -Math.PI / 8 + ((Math.PI / 4) * mousePos.y) / window.innerHeight,
        -Math.PI / 6 + ((Math.PI / 3) * mousePos.x) / window.innerWidth,
        0
      )
    );
  });
  ...
  return (
    <group>
      <Three ref={logo} castShadow rotation={rotationVector} />
      <spotLight
        ref={lightHelper}
        position={[0, 5, 2]}
        color={"#fff"}
